...
class Database:
    def createUpdates(self):
        #   Visitors table holds
        updates = Table('requests', metadata,
                        Column('dns_id', String(256)),
                        Column('name', String(256)),
                        Column('record_type', String(10)),
                        Column('ip_address', String(40)),
                        Column('proxied', String(40)))
        updates.create()
    ...
    def testDatabaseExists(self):
        ...
        if os.path.isfile(util.databaseFileName):
            ...
            print(
                "\t[+]\t\t INFO: Populating database with existing DNS records...\n\n")
            UndoAction().updateDatabase() # FIX UP LATER
        return retVal
